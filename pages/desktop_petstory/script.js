...
const testimonialsArr = document.querySelectorAll(
...
// отображение случайных картинок при старте
showNextPetCards();
...
petCardsNavBtn.forEach((el) =>
  el.addEventListener("click", (event) => showNextPetCards())
);

function showNextPetCards() {
  let newRandomCardsArr = htmlService.randomArrGenerate(
    petCardsArr.length - 1,
    6
  );
